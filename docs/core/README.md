...
git tag -a v0.10.0 -m "Content Creator 0.10.0"
...
```bash
uv run content-creator --workspace . workspace upgrade --to v0.10.0
```

Apply the reviewed preview explicitly:

```bash
uv run content-creator --workspace . workspace upgrade --to v0.10.0 --apply
```
