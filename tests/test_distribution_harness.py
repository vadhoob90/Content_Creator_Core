...
def _sdist(directory: Path, *, omit: str | None = None) -> Path:
    path = directory / "content_creator-1.7.0.tar.gz"
    required = {
        "LICENSE.md",
        "NOTICE",
        "README.md",
    ...
    with tarfile.open(path, "w:gz") as archive:
        for relative in sorted(required - ({omit} if omit else set())):
            payload = b"fixture"
            member = tarfile.TarInfo(f"content_creator-1.7.0/{relative}")
...
def test_distribution_harness_accepts_complete_artifacts(tmp_path):
    ...
    manifest = VALIDATE(tmp_path, "1.7.0")

    assert manifest["package"] == "content-creator"
    assert manifest["version"] == "1.7.0"
    assert set(manifest["artifacts"]) == {
        "content_creator-1.7.0-py3-none-any.whl",
        "content_creator-1.7.0.tar.gz",
    }
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (_metadata(version="9.9.9"), "Expected version 1.7.0"),
...
def test_distribution_harness_rejects_corrupt_archives(tmp_path, artifact):
    _distribution(tmp_path)
    if artifact == "wheel":
        (tmp_path / "content_creator-1.7.0-py3-none-any.whl").write_bytes(b"not a zip")
        error = zipfile.BadZipFile
    else:
        (tmp_path / "content_creator-1.7.0.tar.gz").write_bytes(b"not a tarball")
